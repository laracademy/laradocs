...
class SettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        Setting::create(['setting' => 'github_username', 'parameter' => trim($request->input('github_username'))]);

        if(strlen($request->input('github_token')) > 0)
        {
            Setting::where('setting', 'github_token')->delete();
            Setting::create(['setting' => 'github_token', 'parameter' => trim($request->input('github_token'))]);
        }
